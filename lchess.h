...
//due to bs from ubuntu I had to delete windows compliant parts of the header -.-
...
void printfigure(piece figure){
	...
	printf(" %c ",figure.display);
	reset();
	}else {yellow();printf("|");red();;printf(" %c ",figure.display);reset();}
}
...
int block(position origin,position destination,position pfield[]){
	...
	if(retval!=0){red();printf("Movement blocked fucko!\n");reset();}
	return retval;		
}

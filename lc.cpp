...
//	TILES
	...
	btile.display=' ';		// black tile character as ascii value 177
...
//	BLACK PIECES
	...
	bpawn.display='p';		// display character in ascii value, the colours are separated at printing
...
//		ACTUAL GAME CODE?
	...
	cyan();
	printf("Starting player is cyan, have fun ;)\n\n");
	reset();	
	
	printboard(pfield); //displays the starting board
...
//if user types 1quit' the program ceases to run
	if((co=='q')&&(ro=='u')&&(cd=='i')&&(rd=='t')){system("clear");break;break;}
...
//it would work one cycle off, and would allow movement into check
	...
	printboard(pfield);	//self explanatory but we print the chessboard 

//we notify the players of them being in check

	if(whitecheck!=0){red();printf("Cyan in check! :O\n");reset();}
	if(blackcheck!=0){red();printf("Magenta in check! :O\n");reset();}
